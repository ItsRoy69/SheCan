...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (userSearchByEmail || userSearchByMobile) {
      ...
      if(registered){
        return res.status(201).json({ message: "Registered Successfully!", token : token, professional : professional});
      }
...
router.post("/login", async (req, res) => {
  try {
    ...
    const userByMobile = await Professional.findOne({mobile : logMobile});
...
router.get("/logout", professionalAuth, async (req, res) => {
  ...
  router.get("/get-professional/:id", async (req,res) => {
    ...
    try{
      ...
      if(ProfessionalFound){
        res.status(200).json({message: "Professional Found !", Professional : ProfessionalFound});
      }else{
  ...
  router.get("/get-professional-by-profession", async (req,res) => {
    ...
    try{
      ...
      if(!profession){
        res.status(422).json({message : "Please Provide the specific Profession."})
      }
      const ProfessionalFound = await Professional.find({profession : profession});

      console.log(ProfessionalFound);

      if(ProfessionalFound.length>0){
        res.status(200).json({message: "Professional Found !", Professionals: ProfessionalFound});
      }else{
        res.status(404).json({error : "Could Not Find Professional !"});
      }
    }catch (e) {
