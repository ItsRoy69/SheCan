...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (userSearchByEmail || userSearchByMobile) {
      ...
      if(registered){
      return res.status(201).json({ message: "Registered Successfully!", token : token, customer : customer});
      }
...
router.post("/login", async (req, res) => {
  try {
    ...
    const userByMobile = await Customer.findOne({mobile : logMobile});
...
router.get("/get-customer/:id", async (req,res) => {
  ...
  try{
    ...
    if(CustomerFound){
      res.status(200).json({message: "Customer Found !", Customer : CustomerFound});
    }else{
